...
public class InteractionManager : MonoBehaviour
{
    ...
    /// <summary>
    /// 场景中的交互侦听器列表。 如果列表为空，则将在场景启动时检测可用的交互侦听器。
    /// </summary>
    ...
    void Update()
    {
        ...
        if (kinectManager && kinectManager.IsInitialized())
        {
            ...
            if (playerUserID != 0)
            {
                ...
                //leftIboxLeftBotBack的坐标是(x=2*左臀部的x-右肩的x,y=右臀的y z=脊柱的位置)
                //  leftIboxRightTopFront(x=右肩的x,y=右肩的y z=脊柱的位置-0.5)
                   ...
                   kinectManager.GetJointTrackingState(playerUserID, (int)KinectInterop.JointType.HandLeft) != KinectInterop.TrackingState.NotTracked)
                {
                    ...
                    if (!handOverlayCursor)
                    {
                        ...
                        isLeftHandInteracting = (leftHandPos.x >= (leftIboxLeftBotBack.x - 1.0f)) && (leftHandPos.x <= (leftIboxRightTopFront.x + 0.5f)) &&
                            ...
                            (leftIboxLeftBotBack.z >= leftHandPos.z) && (leftIboxRightTopFront.z * 0.8f <= leftHandPos.z);//当手的位置在这个区域内可以交互
                    }
                    else
                    {
                    ...
                    {
                        leftHandInteractingSince = Time.realtimeSinceStartup;//开始交互计时
                    }
                    ...
                    if (!dragInProgress && isLeftHandInteracting &&
                        ((allowHandClicks && ((leftHandPos - lastLeftHandPos).magnitude < KinectInterop.Constants.ClickMaxDistance)) ||//如果开始点击左手位置减去开始交互时的位置距离不是很远或按压距离大于0则开始算点击开始
