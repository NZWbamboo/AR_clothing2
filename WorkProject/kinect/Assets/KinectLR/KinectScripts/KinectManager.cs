...
{//传感器离地面有多高，单位是米。
    ...
    //屏幕上的深度和彩色图像宽度，如屏幕宽度的百分比。图像高度根据宽度计算。
    ...
    //是只在允许的距离内显示被跟踪的用户，还是显示所有用户(更高的fps)。
    ...
    //是否在LateUpdate()中更新AvatarControllers，而不是在update()中更新。需要为Mecanim动画混合。
...
//设置关节平滑参数
    ...
    //允许手腕和手的旋转:不允许-不允许手的旋转，默认-手的旋转是允许的，除了扭转，所有-所有的旋转都是允许的。
    ...
    //场景中的头像控制器列表。 如果列表为空，则在场景启动时检测可用的头像控制器。
    ...
    //场景中的手势监听器列表。 如果列表为空，则将在场景启动时检测到可用的手势侦听器。
...
//		}
    ...
    /// 返回空间点(或矢量2)的深度映射坐标。如果Kinect没有初始化，则为零
    /// </summary>
